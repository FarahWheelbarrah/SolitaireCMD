...
import CustomExceptions.GiveCardException;
...
public class PlayPile <T> extends Pile implements Giver, Receiver<T> {
    ...
    private boolean canAddToNonEmpty(Card card) {
        return !cards.isEmpty() && card.hasDifference(cards.getLast().getNumber(), 1) && !card.hasColor(cards.getLast().getColor());
    }
    ...
    public void receive(T cardOrCardsToReceive) throws ReceiveCardException {
        if (cardOrCardsToReceive instanceof Card)
            ...
            receive((LinkedList<Card>)cardOrCardsToReceive);
    }
    ...
    private void receive(LinkedList<Card> cardsToReceive) throws ReceiveCardException {
        Card topCard = cardsToReceive.getFirst();
    ...
    private void remove(LinkedList<Card> cardsToRemove) {
    ...
    private String getErrorMessage(LinkedList<Card> cardsToReceive) {
    ...
    private String printOutCards(LinkedList<Card> cardsToReceive) {
        String cardString = "";
